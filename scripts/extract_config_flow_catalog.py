...
plus ``HubEnergieOptionsFlow`` steps (all ``async_step_*`` except ``async_step_expert_back``) for the doc vitrine,
...
# ``step_id`` also exists on the first-time wizard; keep a second catalog row tagged ``HubEnergieOptionsFlow``
# so the vitrine simulator resolves the correct strings when ``mode === 'options'``.
