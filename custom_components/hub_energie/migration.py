...
from .const.tariff_edf import CONF_SITE_SLUG_LOCKED
...
CONFIG_ENTRY_VERSION_SITE_SLUG_IDS = 7
# v8 bridge title uses site slug when set; new ``lovelace_card`` / ``site_ids`` sensors registered on reload.
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version < CONFIG_ENTRY_VERSION_SITE_SLUG_IDS:
        _migrate_entity_ids_for_config_entry(hass, config_entry)
        ...
        hass.config_entries.async_update_entry(
            config_entry,
            version=CONFIG_ENTRY_VERSION_SITE_SLUG_IDS,
        )

    if config_entry.version < CONFIG_ENTRY_VERSION:
        hass.config_entries.async_update_entry(
            config_entry,
            title=hub_energie_bridge_title(dict(config_entry.data)),
