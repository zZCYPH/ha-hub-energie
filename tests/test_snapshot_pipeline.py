...
battery_split_module = importlib.import_module("hub_energie.battery.battery_split")
...
def test_pipeline_eco_solar_uses_self_use_not_full_production() -> None:
    """eco_solar uses PV self-consumption at home, not full production minus double-count."""
    day = "2026-04-22"
    now = datetime(2026, 4, 22, 12, 0, tzinfo=timezone.utc)
    zeros = {slot: 0.0 for slot in SLOTS}
    day_acc = {
        "grid": dict(zeros),
        "solar": {**zeros, "bleu_hp": 10.0},
        "batt_charge": {**zeros, "bleu_hp": 3.0},
        "batt_discharge": dict(zeros),
        "grid_export": {**zeros, "bleu_hp": 2.0},
    }
    rates = {slot: 0.2 for slot in SLOTS}
    ...
    def compute_energy(day_payload: Mapping[str, Any]):
        return energy_module.compute_energy(
            grid={slot: float(day_payload.get("grid", {}).get(slot, 0.0)) for slot in SLOTS},
            solar={slot: float(day_payload.get("solar", {}).get(slot, 0.0)) for slot in SLOTS},
            battery_charge={slot: float(day_payload.get("batt_charge", {}).get(slot, 0.0)) for slot in SLOTS},
            battery_discharge={slot: float(day_payload.get("batt_discharge", {}).get(slot, 0.0)) for slot in SLOTS},
            slots=SLOTS,
        )
    ...
    def compute_savings(solar_by, chg, dis, rates_m):
        return costs_module.compute_savings(
            solar_by_slot=dict(solar_by),
            battery_charge_by_slot=dict(chg),
            battery_discharge_by_slot=dict(dis),
            rates_by_slot=dict(rates_m),
            slots=SLOTS,
    ...
    def usage_heuristic(bc: Mapping[str, float]):
        return battery_split_module.usage_batt_charge_by_slot_from_heuristic(
            batt_charge_by_slot=dict(bc),
            slots=SLOTS,
            is_hc_slot=lambda s: s.endswith("_hc"),
    ...
    deps = pipeline_module.SnapshotPipelineDeps(
        ...
        get_batt_charge_power_split_day=lambda _d: {},
        get_batt_charge_power_split_slot_day=lambda _d: {},
        usage_batt_charge_by_slot_from_heuristic=usage_heuristic,
        compute_tempo_snapshot=compute_tempo,
        compute_battery_metrics=lambda _d: ([], 0.0, 0.0, 0.0, "ok"),
        build_battery_card_metrics=lambda _b: None,
        compute_solar_estimate=lambda _n: (None, None, None),
    ...
    # Maison bleu_hp = 0 + 10 + 0 = 10 kWh (full PV still counts toward house slot energy).
    assert result.snapshot["energy_home_today_kwh"] == pytest.approx(10.0)
    # Self-use = 10 - 3 (solar→batt on HP heuristic) - 2 export = 5 kWh × 0.2 €/kWh = 1.0 €
    assert result.snapshot["eco_solar"] == pytest.approx(1.0)
    assert result.snapshot["eco_batt"] == pytest.approx(0.0)
